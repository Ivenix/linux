...
static int speaker_event(struct snd_soc_dapm_widget *w, struct snd_kcontrol *k, int event)
{
	struct imx_priv *priv = &card_priv;

	if (gpio_is_valid(priv->spk_gpio)) {
		if (SND_SOC_DAPM_EVENT_ON(event))
			gpio_set_value(priv->spk_gpio, !priv->spk_active_low);
...
static const struct snd_soc_dapm_widget imx_tlv320aic3x_dapm_widgets[] = {
	...
	SND_SOC_DAPM_LINE("Line Out", speaker_event),
...
static int imx_tlv320aic3x_probe(struct platform_device *pdev)
{
	...
	priv->spk_gpio = of_get_named_gpio_flags(np, "spk-gpio", 0,
				(enum of_gpio_flags *)&priv->spk_active_low);

	if (gpio_is_valid(priv->spk_gpio)) {
		gpio_request(priv->spk_gpio, "speaker");
		gpio_direction_output(priv->spk_gpio, 0);
	}
