...
#define DS3234_REG_LEN	0x41		/* nb of registers */
...
static ssize_t
ds3234_regmap_read(struct file *filp, struct kobject *kobj,
		...
		char *buf, loff_t off, size_t count)
{
	...
	if (unlikely(off >= DS3234_REG_LEN))
		return 0;
	if (count >= DS3234_REG_LEN)
		count = DS3234_REG_LEN;
	if ((off + count) > DS3234_REG_LEN)
		count = DS3234_REG_LEN - off;
	if (unlikely(!count))
		return count;
...
static ssize_t
ds3234_regmap_write(struct file *filp, struct kobject *kobj,
		struct bin_attribute *attr,
		char *buf, loff_t off, size_t count)
{
	struct spi_device	*spi;
 ...
 */
static int ds3234_rtc_proc(struct device *dev, struct seq_file *seq)
{
	struct spi_device *spi = to_spi_device(dev);
	u8 addr[1], data[DS3234_REG_LEN];
	int status;
	struct spi_message	message;
	struct spi_transfer	x[2];
...
static struct bin_attribute regmap = {
	.attr.name	= "registers",
	.attr.mode	= S_IRUGO | S_IWUSR,
	.read		= ds3234_regmap_read,
	.write		= ds3234_regmap_write,
	.size		= DS3234_REG_LEN,
};
...
static int ds3234_probe(struct spi_device *spi)
{
	...
	/* export registers */
	res = sysfs_create_bin_file(&spi->dev.kobj, &regmap);
	if (res) {
		dev_dbg(&spi->dev, "register mapping failed --> %d\n", res);
...
static int ds3234_remove(struct spi_device *spi)
{
	...
	sysfs_remove_bin_file(&spi->dev.kobj, &regmap);
